...
UDP는 L3에서 쓰던 기능에 port만 추가했다고 봐도 된다. UDP 헤더는 매우 작은 용량으로 Src port, Dst port, Lenght, checksum 각각 2byte씩 총 8byte의 크기를 갖는다. TCP 헤더에도 checksum이 있는데 이 부분은 오류 검사에 사용된다. 그러나 노이즈 체크는 L2에서 이미 하기 떄문에 큰 역할은 없다. L7 계층에서 구현하는 오류 검사 역시 주로 안전한 메시지인지 확인하기 위함이다.
